...
class CMMSParser(object):
    ...
    def _check_and_get_yaml(self):
        ...
        try:
            ...
            if ret.status_code == requests.codes.ok:
                try:
                    self.yaml_content = yaml.safe_load(ret.text)
    ...
    def _parse_yaml_content(self):
        ...
        for yaml_field in self.yaml_content:
            if yaml_field in self.field_mappings:
                ...
                self.errors["field_check"] = (
                    'the field "%s" is not a permitted CMMS field' % yaml_field
                )
    ...
    def _check_and_parse_splice_rules(self):
        ...
        for splice_item_name, splice_rule in self.yaml_content["splice rules"].items():
            ...
            if splice_item_name not in self.yaml_content:
                splice_errors.append(
                    f'"{splice_item_name}" splice rule exists, but no content in yaml file, so not made available'
                )
                continue
    ...
    def _check_and_parse_phenomena(self):
        if isinstance(self.yaml_content["phenomena"], (str, list)):
            ...
            self.errors["phenomena"] = (
                "phenomena needs to be either just one string or a list of entries (which can be strings or dictionaries)"
            )
    ...
    def _check_and_parse_time_range(self):
        ...
        try:
            if isinstance(self.yaml_content["time_range"]["start"], datetime.date):
                self.yaml_content["time_range"]["start"] = datetime.datetime.combine(
                    self.yaml_content["time_range"]["start"], datetime.time(0, 0)
                )
                ...
                if "end" in self.yaml_content["time_range"]:
                    if isinstance(self.yaml_content["time_range"]["end"], datetime.date):
                        self.yaml_content["time_range"]["end"] = datetime.datetime.combine(
                            self.yaml_content["time_range"]["end"],
                    ...
                    if isinstance(self.yaml_content["time_range"]["end"], datetime.datetime):
                        if (
                            self.yaml_content["time_range"]["end"]
                            > self.yaml_content["time_range"]["start"]
                        ):
                            date_bits["endTime"] = self.yaml_content["time_range"]["end"]
                        else:
                            self.errors["time_range"] = "end time preceeds start time!"
                    else:
                        self.errors["time_range"] = (
                            "end time is not in yyyy-mm-dd hh:mm:ss format. No date info supplied."
                        )
            ...
            if access_option == "restricted":
                if "accessRoles" in self.yaml_content:
                    ...
                    if "accessRoles" in self.content:
                        self.content["accessType"] = self.yaml_content[
                            "accessType"
                        ]  # To do: some code is needed here!!!

                else:
                    self.errors["accessType"] = (
                        "access roles are missing - required for accessType = restricted"
                    )
                ...
                self.errors["acesssType"] = (
                    "access roles provided, but not consistent with access types selected"
